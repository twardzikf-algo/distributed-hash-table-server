...
int main(int argc, char** argv) {
  ...
  while (1) {
    ...
    // - Check packet for errors (crcValid), - Check if packet is the one we are expecting
    if ( crcValid == true && data->seqNo == lastReceivedSeqNo+1 ) {
        ...
        if (data->size == sizeof(*data)) {
            ...
            //close the file with fclose and output totalBytes and goodBytes
            fclose(output);
            freeGoBackNMessageStruct(data);
            DEBUGOUT(" totalBytes: %li\n", totalBytes);
            DEBUGOUT(" goodBytes: %li\n", goodBytes);
            break;
        }
    }
    /* END YOUR TASK */
